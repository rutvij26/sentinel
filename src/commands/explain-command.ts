...
export class ExplainCommand implements Command {
  ...
  async execute(context: CommandContext): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to execute explain command: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private parseArgs(commentBody: string): { filename: string | null } {
  ...
  private async postUsageHelp(context: CommandContext): Promise<void> {
    await context.octokit.rest.issues.createComment({
      owner: context.owner,
      repo: context.repository,
      issue_number: context.prNumber,
