...
export class LintCommand implements Command {
  ...
  async execute(context: CommandContext): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to execute lint command: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private buildLintComment(lintIssues: any[]): string {
    let body = '## 🔍 Code Quality Analysis\n\n';

    const issuesBySeverity = {
      error: lintIssues.filter((i: any) => i.severity === 'error'),
      warning: lintIssues.filter((i: any) => i.severity === 'warning'),
      info: lintIssues.filter((i: any) => i.severity === 'info'),
    };

    if (issuesBySeverity.error.length > 0) {
      body += '### ❌ Critical Issues\n';
      issuesBySeverity.error.forEach((issue: any) => {
    ...
    if (issuesBySeverity.warning.length > 0) {
      body += '### ⚠️ Warnings\n';
      issuesBySeverity.warning.forEach((issue: any) => {
        body += `- **${issue.file}:${issue.line}** - ${issue.message}`;
        if (issue.rule) body += ` (${issue.rule})`;
        body += '\n';
      });
    ...
    if (issuesBySeverity.info.length > 0) {
      body += '### ℹ️ Suggestions\n';
      issuesBySeverity.info.forEach((issue: any) => {
        body += `- **${issue.file}:${issue.line}** - ${issue.message}`;
        if (issue.rule) body += ` (${issue.rule})`;
        body += '\n';
      });
